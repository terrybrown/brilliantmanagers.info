...
- **Manager experience** *(May 2026)*: ManagerStrip on the dashboard shows all connected direct reports with real-time round status — "Score now", "Continue scoring", "Complete", or "Self-assessment in progress". Uses the Supabase admin client to bypass RLS so managers can read their DRs' round data. Org/team invite propagation: when a connection is accepted, the new member automatically receives pending invitations to every org node the initiator belongs to. Dismissable tour prompt for both manager and non-manager dashboard views — teal banner with × to permanently hide, plus a topbar "Tour" button that re-triggers the tour at any time.
